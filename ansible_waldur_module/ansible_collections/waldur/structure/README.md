...
#### `customer`
- **Parameter `user_affiliations`:** type changed from `str` to `dict`.
- **Parameter `user_email_patterns`:** type changed from `str` to `dict`.
- **Parameter `user_identity_sources`:** type changed from `str` to `dict`.
#### `project`
- **Parameter `user_affiliations`:** type changed from `str` to `dict`.
- **Parameter `user_email_patterns`:** type changed from `str` to `dict`.
- **Parameter `user_identity_sources`:** type changed from `str` to `dict`.
